...
### UI/mode flow
`selectMode()` drives the top-level UI state machine: hides the welcome screen, reveals the input box, sets the mode badge, and shows a persistent one-line mode confirmation (`showModeConfirm()`/`clearModeConfirm()`) in the chat pane until the first message is sent. `showContextPrompt()`/`submitContext()` handle the one-time Teaching Context widget (discipline/size/modality/level) that's injected as a synthetic user turn before the first real API call. `newConsult()` resets all module-level state and re-renders the welcome screen's markup inline (kept in sync with the initial HTML — if you change the welcome markup, update both copies).
